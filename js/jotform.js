...
{       // Event simulator
    ...
    clearField: function(field) {
        ...
        if(["address", "combined", "datetime", "time"].include(type)) {
            ...
            if(type == "address") {
                if($('input_'+field+'_country') && $('input_'+field+'_country').getAttribute('defaultcountry')){
                    $('input_'+field+'_country').setValue($('input_'+field+'_country').getAttribute('defaultcountry'));
                }
            }
        } else if($('id_'+field).readAttribute('data-type') == "control_paypalpro" || $('id_'+field).readAttribute('data-type') == "control_authnet") {
            ...
            try {
                ...
                var spans = $$("." + field + "_" + calc.replaceText);
                if(spans.length == 0) {
                    var contents = $('text_' + field).innerHTML;
                    var re = new RegExp("\{"+calc.replaceText+"\\[?.*?\\]?\}","g");
                    contents = contents.replace(re, '<span class="'+field + "_" + calc.replaceText+'">'+output+'</span>');
                    $('text_' + field).update(contents);
                } else {
            ...
            if(type == "checkbox" || type == "radio") {
                ...
                if($('input_' + field).hintClear) $('input_' + field).hintClear(); //ie8&9
    ...
    */
    ...
    checkCalculation: function(calc) {
        ...
        var calculationType = function(id) {
        ...
        try {
            if(!['text', 'email', 'textarea', 'calculation', 'combined', 'address', 'datetime', 'time', 'html', 'authnet', 'paypalpro', 'number', 'radio', 'checkbox', 'select'].include(calculationType(result))) return;
        } catch(e) {
            console.log(e);
        }
        ...
        var getValue = function(data, numeric) {
            ...
            if(ignoreHidden && $('id_' + data).hasClassName("form-field-hidden")) {
                return numeric ? 0 : '';
            }

            var type = calculationType(data);
            var val ='';
        ...
        var resultFieldType = calculationType(result);
        ...
        switch(resultFieldType) {
            ...
            case "time":
                ...
                if(resultFieldType == "datetime" && !("year" in combinedObject)) {
                    try {
            ...
            case "checkbox":
                ...
                outputs = outputs.collect(function(out) { return out.strip(); });
